...
/*
   ...
   덱 : https://soft.plusblog.co.kr/24
 */
public class MainDeck {
    ...
    private static Deque<int[]> snakeLocationDeck = new LinkedList<>();
...
//        int endY = startSnakeLocation[1];
        ...
        // 뱀의 현재 위치
        int x = snakeLocationDeck.peekLast()[0];
        int y = snakeLocationDeck.peekLast()[1];
        ...
        if (appleArr[nx][ny] == 1) { // 사과 존재
            ...
            snakeLocationDeck.addLast(new int[]{nx, ny});
        } else {
            /* 사과가 없으면 이동은 되나 꼬리 제거 */
            snakeLocationDeck.addLast(new int[]{nx, ny});
            snakeLocationDeck.pollFirst(); // snake 꼬리 제거
        }
    ...
    private static void input() {
        ...
        /* 뱀의 현재 위치 셋팅 */
        snakeLocationDeck.add(new int[]{1, 1});
